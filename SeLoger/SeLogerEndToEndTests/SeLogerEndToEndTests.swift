...
class SeLogerEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGetPropertyListingsResult_matchesFixedTestAccountData() {
        ...
        case let .success(listings)?:
            ...
            XCTAssertEqual(listings, expectedPropertyListings())
            
        case let .failure(error)?:
            XCTFail("Expected successful property listings result, got \(error) instead")
            
        default:
            XCTFail("Expected successful property listings result, got no instead")
        }
    }
    ...
    private func expectedPropertyListings() -> [PropertyListing] {
